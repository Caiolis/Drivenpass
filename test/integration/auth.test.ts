...
describe('Post /signin', () => {
  ...
  it('should respond status 409 when body is invalid', async () => {
    ...
    const response = await api.post('/users/sign-up').send(invalidBody);
  ...
  describe('When body is valid', () => {
    ...
    it('should respond status 409 when email not exist', async () => {
      ...
      const response = await api.post('/users/sign-in').send(body);
    ...
    it('should respond status 409 when email its wrong', async () => {
      ...
      const response = await api.post('/users/sign-in').send({ email: faker.internet.email(), password: body.password });
      expect(response.status).toBe(httpStatus.CONFLICT);
    });
    ...
    it('should responde status 409 when password is wrong', async () => {
      ...
      const response = await api.post('/users/sign-in').send({
        email: body.email,
        password: faker.internet.password(10),
      });
      expect(response.status).toBe(httpStatus.CONFLICT);
